...
def  dataframe_agent(api_key,df,query):
    model = ChatOpenAI(api_key=api_key,base_url=url,temperature=0)

    agent = create_pandas_dataframe_agent(
        llm=model,df=df,verbose=True,
...
# df = pd.read_csv(r"D:\Projects\Linlili_project\project5\personal_data.csv")
# print(dataframe_agent(key,df,"数据中出现最多的职业是什么？可以逐步思考计算过程"))